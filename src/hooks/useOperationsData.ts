...
export function useOperationsData(enabled: boolean) {
  ...
  const load = useCallback(async () => {
    ...
    const results = await Promise.all([
      ...
      supabase.from('nayax_transactions').select('id,nayax_transaction_id,occurred_at,product_name,quantity,gross_amount,benefit_amount,paid_amount,currency,benefit_type,result,decline_reason,vend_success,employee:employees(first_name,last_name),machine:machines(asset_number)').order('occurred_at',{ascending:false}).limit(500),
      ...
      supabase.from('integration_connections').select('id,organization_id,integration_name,integration_type,status,last_success_at,last_failure_at,last_error,is_active,updated_at').order('integration_name'),
