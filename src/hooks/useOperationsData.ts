...
const empty: OperationsData = { employees: [], cards: [], sites: [], machines: [], tasks: [], incidents: [], transactions: [], memberships: [] };
...
export function useOperationsData(enabled: boolean) {
  ...
  const load = useCallback(async () => {
    ...
    const results = await Promise.all([
      ...
      supabase.from('sites').select('id,client_organization_id,provider_organization_id,code,name,address_line_1,suburb,city,province,client_contact_name,client_contact_email,client_contact_phone,is_active,updated_at').order('name').limit(500),
      supabase.from('machines').select('id,provider_organization_id,client_organization_id,site_id,asset_number,serial_number,display_name,exact_location,status,last_communication_at,last_service_at,next_service_due_at,is_active,site:sites(name),updated_at').order('updated_at',{ascending:false}).limit(500),
    ...
    setData({
      ...
      tasks: results[4].data ?? [], incidents: results[5].data ?? [], transactions: results[6].data ?? [], memberships: results[7].data ?? [],
    });
    setLoading(false);
  }, [enabled]);
