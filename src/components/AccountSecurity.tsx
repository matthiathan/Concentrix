...
export function AccountSecurity({ email, forced = false, onComplete }: Props) {
  ...
  const operations=useOperationsData(!forced&&isAdmin);

  useEffect(()=>{
    if(forced)return;
    void supabase.from('organization_memberships').select('role').eq('is_active',true).then(({data})=>setIsAdmin((data??[]).some(row=>adminRoles.has(row.role))));
  },[forced]);
  ...
  return <div className="administration-workspace">
    <section className="security-panel">
      ...
      <form className="security-form" onSubmit={handleSubmit}>
        <label><span>New password</span><div className="auth-input"><KeyRound size={18}/><input type="password" value={password} onChange={event => setPassword(event.target.value)} autoComplete="new-password" required minLength={forced ? 12 : 8}/></div></label>
        <label><span>Confirm new password</span><div className="auth-input"><KeyRound size={18}/><input type="password" value={confirmPassword} onChange={event => setConfirmPassword(event.target.value)} autoComplete="new-password" required minLength={forced ? 12 : 8}/></div></label>
    ...
    {!forced&&isAdmin&&!operations.loading&&<IntegrationControlPage data={operations.data} error={operations.error} refresh={operations.refresh}/>} 
  </div>;
}
